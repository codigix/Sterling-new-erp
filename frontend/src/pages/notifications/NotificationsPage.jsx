  ...
  return (
    <div className="task-page-container w-full mx-auto p-4 px-4 py-8">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
        <div>
          <h1 className="text-xl  text-slate-900 dark:text-white flex items-center gap-3 mb-2">
          ...
          <p className="text-slate-500 text-xs dark:text-slate-400 ">
            ...
            {unreadCount > 0 && (
              <span className="ml-2 p-2 bg-red-100 text-red-600 rounded  text-xs ">
                {unreadCount} Unread
              </span>
            )}
          </p>
        ...
        <div className="flex items-center gap-3">
          ...
          {unreadCount > 0 && (
            <button
              ...
              className="flex items-center gap-2 p-2.5 rounded bg-emerald-600 text-white  text-sm hover:bg-emerald-700 transition-all  shadow-emerald-200 dark:shadow-none"
            >
              <CheckCheck size={15} />
              Mark All Read
            </button>
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 sm:pb-0 custom-scrollbar flex-1">
          {types.map((type) => (
            <button
              ...
              className={`p-2 rounded  text-xs    transition-all whitespace-nowrap border ${
                filterType === type
          ...
          {dateFilter === "custom" && (
            <div className="flex items-center gap-2 bg-white dark:bg-slate-800 p-1 rounded border border-slate-200 dark:border-slate-700 animate-in fade-in slide-in-from-right-2">
              <input
                ...
                className="bg-transparent text-xs text-slate-600 dark:text-slate-300 outline-none p-1 cursor-pointer"
                title="Start Date"
              />
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="p-2 flex items-start gap-5">
                        <div
                          className={`w-5 h-5 rounded  flex items-center justify-center flex-shrink-0 ${styles.bg}`}
                        >
                          {styles.icon}
                        </div>
                          ...
                          <div className="flex flex-col md:flex-row md:items-start justify-between gap-2">
                            <div>
                              <div className="flex items-center gap-2 mb-1">
                                <span
                                  className={`p-1 rounded text-xs    ${styles.badge}`}
                                >
                                  {notif.alert_type?.replace("_", " ")}
                                </span>
                                {notif.priority === "high" && (
                                  <span className="flex items-center gap-1 text-xs  text-red-600 bg-red-50 p-2 rounded ">
