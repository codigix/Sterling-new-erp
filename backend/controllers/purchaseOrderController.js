  ...
  try {
    ...
    if (inventory_status && inventory_status !== "all") {
      if (inventory_status === "!fulfilled") {
        query +=
          " AND (po.inventory_status IS NULL OR po.inventory_status != 'fulfilled')";
      } else {
    ...
    if (search) {
      query +=
        " AND (po.po_number LIKE ? OR v.name LIKE ? OR q.quotation_number LIKE ?)";
...
const getPurchaseOrderById = async (req, res) => {
  ...
  try {
    ...
    const [attachments] = await db.query(
      "SELECT * FROM purchase_order_attachments WHERE purchase_order_id = ?",
      [po.id],
    );
    ...
    if (items && items.length > 0) {
      const itemValues = items.map((item) => {
        ...
        return [
          ...
          finalRate,
          item.amount ||
            item.total_weight * (item.rate_per_kg || 0) ||
            finalQty * finalRate,
          item.length || null,
          item.width || null,
          item.thickness || null,
...
const updatePurchaseOrderStatus = async (req, res) => {
  ...
  try {
    if (status && inventory_status) {
      ...
      await db.query(
        "UPDATE purchase_orders SET inventory_status = ? WHERE id = ?",
        [inventory_status, id],
      );
    }
    ...
    if (status === "sent to inventory") {
      ...
      if (rows.length > 0) {
        ...
        if (poData.root_card_id) {
          await db.query(
            'UPDATE root_cards SET status = "PURCHASE_ORDER_RELEASED" WHERE id = ?',
            [poData.root_card_id],
          );
        }
...
const getCommunications = async (req, res) => {
  ...
  try {
    ...
    await db.query(
      "UPDATE purchase_order_communications SET is_read = TRUE WHERE purchase_order_id = ? AND is_outgoing = FALSE",
      [id],
    );
...
const downloadAttachment = async (req, res) => {
  ...
  try {
    ...
    if (rows.length === 0) {
      return res
        .status(404)
        .json({ message: "Attachment not found in database" });
    }
...
const uploadInvoice = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({
      message:
        isFullReceipt === "true" || isFullReceipt === true
  ...
  try {
    ...
    if (recentGRN.length > 0) {
      ...
      return res
        .status(409)
        .json({
          message: "A GRN was already created for this PO a few seconds ago.",
        });
    }
    ...
    while (!isGrnUnique) {
      ...
      const [existing] = await connection.query(
        "SELECT id FROM grns WHERE grn_number = ?",
        [grn_number],
      );
    ...
    for (const item of items) {
      const { po_item_id, material_name, received_qty, unit, generate_st } =
        item;
      ...
      if (poItemRows.length > 0) {
        ...
        if (parseFloat(received_qty) > remaining + 0.001) {
          // 0.001 for decimal precision
          await connection.rollback();
      ...
      const generateItemCode = (name) => {
        ...
        if (upperName.includes("PLATE")) typeCode = "PLT";
          ...
          upperName.includes("ROUND BAR") ||
          upperName.includes("RB") ||
          upperName.includes("Ø") ||
          upperName.includes("DIA")
        )
          typeCode = "RB";
          ...
          upperName.includes("WELDING") ||
          upperName.includes("ELECTRODE")
        )
          typeCode = "WLD";
        ...
        if (sizeMatch) {
          ...
          sizeMatch = upperName.match(
            /(?:Ø|DIA|RB|ROUND BAR)?\s*(\d+)\s*[X]\s*(\d+)/,
          );
          if (sizeMatch) {
    ...
    await connection.query(
      "UPDATE purchase_orders SET inventory_status = ? WHERE id = ?",
      [inventoryStatus, purchase_order_id],
    );
...
const getPurchaseReceiptById = async (req, res) => {
  ...
  try {
    ...
    const itemsWithSerials = items.map((item) => ({
      ...
      serials: serials
        .filter((s) => s.item_id === item.po_item_id)
        .map((s) => ({
          ...s,
...
const addGRNToStock = async (req, res) => {
  ...
  try {
    ...
    const [lastEntry] = await connection.query(
      "SELECT entry_no FROM stock_entries ORDER BY id DESC LIMIT 1",
    );
    let nextNum = "0001";
    if (
      lastEntry.length > 0 &&
      lastEntry[0].entry_no.startsWith(`STE-${year}`)
    ) {
    ...
    for (const item of items) {
      ...
      const newBalance =
        parseFloat(currentBalance) + parseFloat(item.received_qty);
    ...
    await connection.query('UPDATE grns SET status = "pending" WHERE id = ?', [
      id,
    ]);

    await connection.commit();
    res.json({
      message: "Material added to stock and stock entry created successfully",
      entry_no,
    });
  } catch (error) {
    await connection.rollback();
...
const releaseGRNMaterial = async (req, res) => {
  ...
  try {
    ...
    if (grn.status !== "qc_completed") {
      throw new Error(
        'GRN must be in "QC Completed" status to release material',
      );
    }
    ...
    const itemStats = items.map((item) => {
      ...
      if (isNos) {
        acceptedQty = itemSerials.filter(
          (s) => s.inspection_status === "Accepted",
        ).length;
        rejectedQty = itemSerials.filter(
          (s) => s.inspection_status === "Rejected",
        ).length;
      } else {
        // For non-Nos, it's usually all or nothing per serial (and there's only 1 serial)
        const hasAccepted = itemSerials.some(
          (s) => s.inspection_status === "Accepted",
        );
        const hasRejected = itemSerials.some(
          (s) => s.inspection_status === "Rejected",
        );
      ...
      return {
        ...
        shortage_to_order: Math.max(
          0,
          parseFloat(item.ordered_qty || 0) - acceptedQty,
        ),
      };
    });
    ...
    const [entryResult] = await connection.query(
      ...
      [
        ...
        "Material Issue",
        new Date().toISOString().split("T")[0],
        `Released for Production from ${grn.grn_number}`,
        id,
        grn.project_name || null,
        grn.vendor_name || null,
        "submitted",
    ...
    for (const item of itemStats) {
      ...
      if (item.accepted_qty > 0) {
        ...
        const currentBalance = lastBalance[0]?.balance_qty || 0;
        const newBalance =
          parseFloat(currentBalance) - parseFloat(item.accepted_qty);
    ...
    const finalStatus =
      totalShortageCount > 0 ? "partially_released" : "material_released";
    await connection.query("UPDATE grns SET status = ? WHERE id = ?", [
      finalStatus,
      id,
    ]);
    ...
    if (grn.root_card_id) {
      const rcStatus =
        finalStatus === "partially_released"
          ? "PARTIALLY_RELEASED"
          : "MATERIAL_RELEASED";
      await connection.query("UPDATE root_cards SET status = ? WHERE id = ?", [
        rcStatus,
        grn.root_card_id,
      ]);
    }
    ...
    if (shortageItems.length > 0) {
      ...
      const bomNumber =
        grn.active_bom_number || grn.original_bom_number || grn.grn_number;
      ...
      const [mrResult] = await connection.query(
        ...
        [
          ...
          "Procurement",
          grn.project_id || null,
          grn.root_card_id || null,
          req.user?.id || null,
          `Shortage from GRN ${grn.grn_number} (PO: ${grn.po_number})`,
          grn.project_name,
          "shortage",
          bomNumber,
        ],
