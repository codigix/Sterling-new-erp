...
const AccordionSection = memo(
  ({
    ...
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded  overflow-hidden transition-all duration-200">
      <div
        className={`p-2 flex items-center justify-between cursor-pointer select-none transition-colors ${expandedSections[section] ? "bg-slate-50/80 dark:bg-slate-800/50" : "hover:bg-slate-50/50 dark:hover:bg-slate-800/30"}`}
        onClick={() => toggleSection(section)}
      >
        <div className="flex items-center gap-2">
          <div
            className={`p-2 rounded transition-colors ${expandedSections[section] ? "bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400" : "bg-slate-100 dark:bg-slate-800 text-slate-500"}`}
          >
            <ChevronDown
              size={15}
          ...
          <h3 className="text-sm  text-slate-900 dark:text-white">{title}</h3>
          {itemCount > 0 && (
...
const CreateBOMPage = () => {
    ...
    (section) => {
      ...
      if (section === "materials") {
        ...
        newItem = {
          ...
          totalWeight:
            (parseFloat(newMaterial.calculatedWeight) || 0) *
  ...
  const loadRootCardContext = useCallback(async (rootCardId) => {
    try {
      const [rcRes, reqRes] = await Promise.all([
        ...
        axios
          .get(`/root-cards/requirements/${rootCardId}`)
      ...
      if (rootCard.steps) {
        if (rootCard.steps.client_po)
          rootCard.steps.step1_clientPO = rootCard.steps.client_po;
        if (rootCard.steps.design_engineering)
          rootCard.steps.step2_design = rootCard.steps.design_engineering;
        if (rootCard.steps.procurement)
          rootCard.steps.step3_procurement = rootCard.steps.procurement;
        if (rootCard.steps.production)
          rootCard.steps.step4_production = rootCard.steps.production;
      }

      // --- REQUIREMENTS ---
      let potentialMaterials = [];
      if (reqRes.data?.success && reqRes.data?.data) {
        potentialMaterials = (reqRes.data.data.materials || []).map((req) => ({
          ...req,
      ...
      const designEngineering =
        rootCard.steps?.step2_design || rootCard.designEngineering;
    ...
    async (id) => {
      try {
        ...
        const response = await axios.get(
          `/engineering/bom/comprehensive/${id}`,
        );
        ...
        Swal.fire({
          icon: "error",
          title: "Error",
          text: "Failed to fetch BOM details",
        });
      } finally {
  ...
  const handleRootCardSelect = useCallback(
    async (rootCardId) => {
      ...
      try {
        ...
        if (rootCard.steps?.step1_clientPO?.productDetails) {
          ...
          try {
            details =
              typeof rootCard.product_details === "string"
        ...
        Swal.fire({
          icon: "error",
          title: "Error",
          text: "Failed to fetch route card details",
        });
      } finally {
        setLoadingMaterials(false);
      }
    ...
    const fetchData = async () => {
      try {
        ...
        const [rootCardsRes, bomsRes] = await Promise.all([
          ...
          axios
            .get("/engineering/bom/comprehensive")
            .catch(() => ({ data: { boms: [] } })),
        ]);
    ...
    try {
      ...
      if (editMode && bomId) {
        response = await axios.put(
          `/engineering/bom/comprehensive/${bomId}`,
          payload,
        );
      } else {
          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-3  gap-3">
              ...
              <div className="space-y-1">
                <label
                  htmlFor="bomNumber"
                  className="block text-xs  text-slate-900 dark:text-slate-100"
                >
                  BOM Number
                </label>
                <input
            ...
            <p className="text-xs  text-slate-500 dark:text-slate-400 mt-2 flex items-center gap-1.5">
              ...
              Selecting a route card will automatically populate product
              information and link this BOM to its project.
            </p>
          </AccordionSection>
            ...
            {!isRootCardSelected ? (
              <div className="text-center py-12 bg-slate-50/50 dark:bg-slate-800/20 rounded border-2 border-dashed border-slate-200 dark:border-slate-800">
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 max-w-xs mx-auto">
                  Please select a Route Card in Product Information to start
                  adding materials.
                </p>
              </div>
            ) : (
              <div className="space-y-2">
                ...
                <div className="p-2 bg-emerald-50/30 dark:bg-emerald-900/10 rounded border border-emerald-100/50 dark:border-emerald-900/20">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                            ...
                            onChange={(val) => {
                              if (val === "other") {
                                setNewMaterial((prev) => ({
                                  ...
                                  densityType: "other",
                                  density:
                                    prev.densityType === "other"
                                      ? prev.density
                                      : 0,
                                  materialType: "Other",
                                }));
                              } else {
                                const selected = MaterialTypeOptions.find(
                                  (opt) => opt.value === val,
                                );
                                setNewMaterial((prev) => ({
                                  ...prev,
                    ...
                    {(newMaterial.itemGroup?.toLowerCase().includes("i beam") ||
                      newMaterial.itemGroup
                        ?.toLowerCase()
                        .includes("h beam")) && (
                      <>
                        ...
                        <div className="md:col-span-2">
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setNewMaterial((prev) => ({
                                ...prev,
                                flangeThickness: e.target.value,
                              }))
                            }
                    ...
                    <div className="md:col-span-3">
                      <SearchableSelect
                        ...
                        onChange={(val) =>
                          setNewMaterial((prev) => ({ ...prev, uom: val }))
                        }
                        allowCustom={true}
                      />
                    </div>
                        ...
                        <div className="flex gap-6">
                          <div>
                            ...
                            <span className="text-sm  text-blue-700 dark:text-blue-400">
                              {Number(newMaterial.calculatedWeight.toFixed(3))}{" "}
                              Kg
                            </span>
                          </div>
                          <div>
                          ...
                          <div>
                            ...
                            <span className="text-sm  text-blue-700 dark:text-blue-400">
                              {newMaterial.materialType} ({newMaterial.density})
                            </span>
                          </div>
                        </div>
                        {newMaterial.uom === "Kg" && (
                          <button
                            ...
                            onClick={() =>
                              setNewMaterial((prev) => ({
                                ...
                                quantity: Number(
                                  (
                                    prev.calculatedWeight *
                                    (parseFloat(prev.quantity) || 1)
                                  ).toFixed(3),
                                ),
                              }))
                            }
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end mt-4"></div>
                </div>
                  ...
                  <table className="w-full text-xs text-left border-collapse">
                    <thead>
                      <tr className="bg-slate-50/80 dark:bg-slate-800/50">
                        <th className="p-2  text-slate-500 dark:text-slate-400   w-10">
                          #
                        </th>
                        <th className="p-2  text-slate-500 dark:text-slate-400  ">
                          Item Name / Group
                        </th>
                        <th className="p-2  text-slate-500 dark:text-slate-400  ">
                          Part Detail / Grade
                        ...
                        <th className="p-2  text-slate-500 dark:text-slate-400   text-center">
                          Weight (Kg)
                        </th>
                        <th className="p-2  text-slate-500 dark:text-slate-400   text-center">
                          Qty
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                      {bomData.materials.map((row, index) => {
                        ...
                        return (
                          <tr
                            key={row.id}
                            className="bg-white dark:bg-slate-900 hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors"
                          >
                                ...
                                <div className="flex flex-col">
                                  ...
                                  {renderDimensions(row) !== "-" && (
                                    <div className="flex flex-wrap gap-1 mt-1 mb-1">
                                      {renderDimensions(row).split(" x ").map((part, pIdx) => (
                                        <span 
                                          key={pIdx} 
                                          className="text-[10px] font-mono font-medium text-blue-700 dark:text-blue-300 bg-blue-50 dark:bg-blue-900/30 px-1.5 py-0.5 rounded border border-blue-100 dark:border-blue-800/50 whitespace-nowrap"
                                        >
                                          {part}
                                        </span>
                                      ))}
                              ...
                              <div className="flex items-center justify-center gap-1">
                                {isEditing ? (
                                  ...
                                  <>
                                    ...
                                    <button
                                      onClick={() =>
                                        removeTableRow("materials", row.id)
                                      }
                                      className="p-1.5 text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-900/20 rounded transition-colors"
                                      title="Delete Row"
                                    >
                ...
                {bomData.materials.length === 0 && (
                  <div className="text-center py-6 text-slate-400 text-xs  italic">
                    No materials added yet. Use the form above to add your first
                    material.
                  </div>
                )}
              </div>
          ...
          <div className="flex items-center gap-3">
            ...
            {/* {editMode && (
              <button
                ...
                disabled={saving}
                className="p-2 bg-blue-600 hover:bg-blue-700 text-xs text-white rounded  shadow-lg shadow-blue-600/20 transition-all active:scale-95 flex items-center gap-2"
              >
                <Copy size={15} />
