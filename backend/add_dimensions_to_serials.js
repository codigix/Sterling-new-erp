...
const addDimensionsToSerials = async () => {
    try {
        const table = 'inventory_serials';
        const columns = [
        ...
        console.log('Inventory serials table update completed');
        process.exit(0);
    } catch (error) {
        console.error('Failed to update inventory serials table:', error);
        process.exit(1);
    }
};

addDimensionsToSerials();
