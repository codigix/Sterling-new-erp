  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
        <div>
          <h2 className="text-lg font-bold text-slate-900 dark:text-white">Released Production Projects</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {releasedProjects.map((project) => (
          <div key={project.id} className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden hover:shadow-md transition-shadow">
            <div className="p-5">
              ...
              <h3 className="text-sm font-bold text-slate-900 dark:text-white mb-1 line-clamp-1">{project.name}</h3>
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                <div className="bg-slate-50 dark:bg-slate-900/50 p-2.5 rounded-lg border border-slate-100 dark:border-slate-700">
                  <p className="text-[10px] text-slate-400 uppercase font-bold tracking-widest mb-0.5">Quantity</p>
                  <p className="text-sm font-bold text-slate-900 dark:text-white">{project.qty} Units</p>
                </div>
                <div className="bg-slate-50 dark:bg-slate-900/50 p-2.5 rounded-lg border border-slate-100 dark:border-slate-700">
                  <p className="text-[10px] text-slate-400 uppercase font-bold tracking-widest mb-0.5">Planning</p>
                  <p className="text-sm font-bold text-slate-900 dark:text-white">{project.progress}% Done</p>
                </div>
              </div>
              
              <button className="w-full flex items-center justify-center gap-2 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-bold transition-colors">
        ...
        <button className="bg-slate-50 dark:bg-slate-900/30 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-xl p-6 flex flex-col items-center justify-center text-slate-400 hover:text-blue-600 hover:border-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/10 transition-all group">
          ...
          <span className="text-sm font-bold">Import More Projects</span>
          <span className="text-[10px] uppercase tracking-widest mt-1">From Sales/Release</span>
        </button>
      </div>
      ...
      <div className="bg-blue-50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/20 rounded-xl p-4 flex gap-3">
        ...
        <div>
          <h4 className="text-sm font-bold text-blue-900 dark:text-blue-300">Planning Guidelines</h4>
