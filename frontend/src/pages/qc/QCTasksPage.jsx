                  ...
                  <div className="flex items-center gap-2">
                    {!report.is_sent_to_inventory ? (
                      <button 
                        ...
                        className={`px-4 py-2 bg-blue-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-blue-700 transition-all shadow-lg shadow-blue-200 flex items-center gap-2 ${sendingReport === report.id ? 'opacity-50 cursor-not-allowed' : ''}`}
                      >
                    ...
                    <button 
                      ...
                      className="p-2.5 bg-slate-50 dark:bg-slate-900 text-slate-400 hover:text-red-600 rounded-xl transition-all"
                      title="Delete Report"
                    >
                      <Trash2 size={18} />
                    </button>
                                ...
                                >
                                  ...
                                  <td className="px-4 py-4">
                                    <div className="flex items-center justify-center gap-2">
                                      <span className="px-2 py-0.5 bg-green-50 text-green-600 rounded-lg border border-green-100 flex items-center gap-1">
                                        <CheckCircle size={10} /> {m.accepted_qty} Accepted
                                      </span>
                                      {m.rejected_qty > 0 && (
                                        <span className="px-2 py-0.5 bg-red-50 text-red-600 rounded-lg border border-red-100 flex items-center gap-1">
                                          ...
                                          >
                                            <div className="flex items-center justify-between">
                                              <div className="flex items-center gap-1.5">
                                                <div className={`w-2 h-2 rounded-full ${st.status === 'ACCEPTED' ? 'bg-green-500 animate-pulse' : 'bg-red-500 animate-pulse'}`}></div>
                                                <span className={`text-[10px] font-black uppercase tracking-tight ${st.status === 'ACCEPTED' ? 'text-green-600' : 'text-red-600'}`}>
                                                  {st.status}
                                                </span>
                                              </div>
                                              <div className="flex items-center gap-1.5">
                                            ...
                                            <div className="space-y-1">
                                              <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest">ST Number (Serial)</p>
                                              <p className="text-[11px] font-black text-slate-700 dark:text-slate-200 uppercase tracking-tight font-mono truncate" title={st.st_code}>
                                                {st.st_code}
                                              </p>
                                            </div>
                                            ...
                                            <div className="grid grid-cols-2 gap-2 pt-2 border-t border-slate-50 dark:border-slate-800">
                                              <div>
                                                <p className="text-[7px] font-bold text-slate-400 uppercase tracking-tighter">Item Code</p>
                                                <p className="text-[9px] font-black text-slate-600 dark:text-slate-400 truncate">{st.item_code || m.item_code || 'N/A'}</p>
                                              </div>
                                              <div>
                                                <p className="text-[7px] font-bold text-slate-400 uppercase tracking-tighter">Material Name</p>
                                                <p className="text-[9px] font-black text-slate-600 dark:text-slate-400 truncate">{m.material_name}</p>
                                              </div>
                                            </div>
                                          </div>
                                        ))}
